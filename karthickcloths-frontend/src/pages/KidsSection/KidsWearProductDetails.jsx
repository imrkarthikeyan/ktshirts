...
import { createWhatsappOrderLink, fetchKidsProductById } from "../../services/api";
import { kidsProductsFallback } from "./kidsProductsData";
import { useAuth } from "../../context/AuthContext";
import { useCart } from "../../context/CartContext";
import { useWishlist } from "../../context/WishlistContext";

function KidsWearProductDetails({ isDark }) {
    ...
    useEffect(() => {
        ...
        const loadProduct = async () => {
            try {
                const data = await fetchKidsProductById(productId);
                if (isMounted) {
                    setProduct(data);
                    setSelectedColor(data.defaultColor || data.availableColors?.[0] || "Black");
                }
            } catch (error) {
                const fallbackProduct = kidsProductsFallback.find((item) => item.id === productId);
                if (isMounted) {
                    ...
                    <div className="slide-in-right">
                        ...
                        <div className="mt-2 flex items-start justify-between gap-3">
                            ...
                            <button
                                ...
                                className={`wishlist-heart-btn mt-1 ${isWishlisted(product.id, "kids") ? "wishlist-heart-active" : ""}`}
                                aria-label="Toggle wishlist"
                            >
                                {isWishlisted(product.id, "kids") ? "♥" : "♡"}
                            </button>
                        </div>
                        ...
                        <div className={isDark ? "mt-8 rounded-2xl border border-zinc-800 bg-zinc-950 p-5" : "mt-8 rounded-2xl border border-zinc-200 bg-zinc-50 p-5"}>
                            ...
                            {showSizeChart && (
                                <div className={isDark ? "mt-3 rounded-xl border border-zinc-700 p-3 text-xs text-zinc-300" : "mt-3 rounded-xl border border-zinc-300 p-3 text-xs text-zinc-700"}>
                                    <p>3-4Y: Height 95-105 cm</p>
                                    <p>5-6Y: Height 105-115 cm</p>
                                    <p>7-8Y: Height 115-125 cm</p>
                                    <p>9-10Y: Height 125-135 cm</p>
                                </div>
                            )}
...
export default KidsWearProductDetails;
